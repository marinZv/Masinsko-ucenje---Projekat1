...
# Pomocna funkcija koja od niza trening primera pravi feature matricu (m X n).
def create_feature_matrix1(x, nb_features):
  tmp_features = []
  for deg in range(1, nb_features+1):
    tmp_features.append(np.power(x, deg))
  return np.column_stack(tmp_features)
...
# Učitavanje i obrada podataka.
filename1 = 'funky.csv'
all_data1 = np.loadtxt(filename, delimiter=',', usecols=(0, 1), dtype='float32')
data1 = dict()
data1['x'] = all_data1[:, 0]
data1['y'] = all_data1[:, 1]

# Nasumično mešanje.
nb_samples1 = data1['x'].shape[0]
indices1 = np.random.permutation(nb_samples1)
data1['x'] = data1['x'][indices1]
data1['y'] = data1['y'][indices1]

# Normalizacija (obratiti pažnju na axis=0).
data1['x'] = (data1['x'] - np.mean(data1['x'], axis=0)) /
np.std(data1['x'], axis=0)
data1['y'] = (data1['y'] - np.mean(data1['y'])) / np.std(data1['y'])
...
print(data1['x'][:3])
print('Feature matrica (prva 3 reda):')
data1['x'] = create_feature_matrix1(data1['x'], nb_features1)
print(data1['x'][:3, :])

# Iscrtavanje.
plt.scatter(data1['x'][:, 0], data1['y'])

# Model i parametri.
w1 = tf.Variable(tf.zeros(nb_features1))
b1 = tf.Variable(0.0)
...
def pred(x, w, b):
    w_col = tf.reshape(w, (nb_features1, 1))
    hyp = tf.add(tf.matmul(x, w_col), b)
    return hyp
...
# # Prelazimo na AdamOptimizer jer se prost GradientDescent lose snalazi sa
# # slozenijim funkcijama.
adam1 = tf.keras.optimizers.Adam(learning_rate=learning_rate)

# # Trening korak
def train_step(x, y, w, b):
    w_grad, b_grad, loss = calc_grad(x, y, w, b)

    adam1.apply_gradients(zip([w_grad, b_grad], [w, b]))
...
for epoch in range(nb_epochs1):
    ...
    # Stochastic Gradient Descent.
    epoch_loss = 0
    for sample in range(nb_samples1):
        x1 = data1['x'][sample].reshape((1, nb_features1))
        y1 = data1['y'][sample]

        curr_loss = train_step(x1, y1, w1, b1)
        epoch_loss += curr_loss

    # U svakoj stotoj epohi ispisujemo prosečan loss.
    epoch_loss /= nb_samples1
    if (epoch + 1) % 100 == 0:
      print(f'Epoch: {epoch+1}/{nb_epochs}| Avg loss: {epoch_loss:.5f}')

curr_loss=0
for sample in range(nb_samples):
    x = data1['x'][sample].reshape((1, nb_features1))
    y = data1['y'][sample]
    curr_loss+=train_step(x,y,w1,b1)

costs.append(curr_loss)

# Ispisujemo i plotujemo finalnu vrednost parametara.
print(f'w = {w1.numpy()}, bias = {b1.numpy()}')
xs1 = create_feature_matrix1(np.linspace(-2, 4, 100, dtype='float32'),
nb_features1)
hyp_val1 = pred(xs1, w1, b1)
plt.plot(xs1[:, 0].tolist(), hyp_val1.numpy().tolist(), color='r')
plt.xlim([-3, 3])
plt.ylim([-3, 3])
